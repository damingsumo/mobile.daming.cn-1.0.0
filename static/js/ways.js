...
//改变身高体重
   ...
   //人的模型
    ...
    'images/zhuilinglian.png','images/chaoduanfa1.png'];
 	$(".aa").bind("myClick", function(){
 		base64=[];
        draw(function(){
        document.getElementById('aa').innerHTML='<img src="'+base64[0]+'" id="img">';
        }) 
    function draw(fn){
        ...
        function drawing(n){
            if(n<len){
                ...
                img.onload=function(){
                    ...
                    //  else {
                    // 	ctx.drawImage(img,35,71,100,190);//内衬1
                    //      drawing(n+1);//递归
                    // } 
                }
            }else{
    ...
    function draw(fn){
        ...
        function drawing(n){
            if(n<len){
                ...
                img.onload=function(){
                     if(n==0){
                     	  ctx.drawImage(img,25,0,160,300);//头发
                         drawing(n+1);//递归

                   }
                   else if(n==1){
                    	ctx.drawImage(img,5,64,86,130);//内衬1
                         drawing(n+1);//递归
                    } 
                    else{
                    	ctx.drawImage(img,76,270,35,43);//内衬1
                         drawing(n+1);//递归
                    }
                }
            }else{
