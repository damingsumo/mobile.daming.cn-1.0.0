...
class pager{
	...
	public function __construct($mixTotal=null,$page=null,$pageSize=null,$showNumber=null,$type="default"){
		$this->type = $type;
		if (is_object($mixTotal) && $mixTotal instanceof stdClass && 
				isset($mixTotal->total) && isset($mixTotal->page) && isset($mixTotal->pageSize)) {
			...
			if(is_numeric($mixTotal) && $mixTotal>0)	 $this->total = (integer)$mixTotal;
			if(is_numeric($pageSize) && $pageSize>0)  	 $this->pageSize = (integer)$pageSize;
			if(is_numeric($showNumber) && $showNumber>0) $this->showNumber = (integer)$showNumber;
	...
	private function getPageList(){
		...
		if ($first) {
			$results = $this->formatHTML($this->makeLinkHTML(1,1),$this->styleLink).$this->listSeparator
						.$this->formatHTML($this->makeLinkHTML("...",$start-1),$this->styleMore)
							.$this->listSeparator.$results;
		}
		if ($last) {
			$results .= $this->listSeparator;
